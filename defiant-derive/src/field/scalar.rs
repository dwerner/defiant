...
impl Field {
    ...
    pub fn encode(&self, prost_path: &Path, ident: TokenStream) -> TokenStream {
        ...
        // All types use regular encode functions
        ...
        match self.kind {
            Kind::Plain(ref default) => {
                ...
                // For string/bytes types (&str, &[u8]), pass value directly (already a reference)
            ...
            Kind::Repeated | Kind::Packed => {
                // For repeated strings/bytes, field is &[&str] or &[&[u8]], pass directly
                // For other types, field is &[T], need to take reference
                let param = if matches!(self.ty, Ty::String | Ty::Bytes(_)) {
                    quote!(#ident)
                } else {
                    quote!(&#ident)
    ...
    pub fn encoded_len(&self, prost_path: &Path, ident: TokenStream) -> TokenStream {
        ...
        // All types use regular encoded_len functions
