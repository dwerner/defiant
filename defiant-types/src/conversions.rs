...
use crate::String;
use crate::Vec;
use defiant::alloc::collections::BTreeMap;
...
impl<'arena> defiant::ArenaFrom<'arena, String> for Value<'arena> {
    fn arena_from(value: String, arena: &'arena defiant::Arena) -> Self {
        let s = arena.alloc_str(&value);
        value::Kind::StringValue(s).into()
    }
}

impl<'arena> defiant::ArenaFrom<'arena, &str> for Value<'arena> {
    fn arena_from(value: &str, arena: &'arena defiant::Arena) -> Self {
        let s = arena.alloc_str(value);
        value::Kind::StringValue(s).into()
    }
}
...
impl<'arena> defiant::ArenaFrom<'arena, BTreeMap<String, Value<'arena>>> for Value<'arena> {
    fn arena_from(map: BTreeMap<String, Value<'arena>>, arena: &'arena defiant::Arena) -> Self {
        // Convert BTreeMap to arena map, allocating String keys into arena as &str
        let mut vec: defiant::ArenaVec<'arena, (&'arena str, Value<'arena>)> = arena.new_vec();
