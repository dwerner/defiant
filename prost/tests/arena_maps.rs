...
impl<'arena> Message<'arena> for UserProfileBuilder {
    ...
    fn encode_raw(&self, buf: &mut impl BufMut) {
        if !self.username.is_empty() {
            string::encode(1, &self.username, buf);
        }
    ...
    fn merge_field(
        ...
        match tag {
            1 => {
                string::merge(wire_type, &mut self.username, buf, ctx)
            }
            2 => {
                // Decode map entry
    ...
    fn encoded_len(&self) -> usize {
        let mut len = 0;
        if !self.username.is_empty() {
            len += string::encoded_len(1, &self.username);
        }
        for entry in &self.metadata {
...
impl UserProfileBuilder {
    fn into_arena<'arena>(self, arena: &'arena Arena) -> UserProfile<'arena> {
        ...
        UserProfile {
            ...
            metadata: arena.alloc_slice_copy(&metadata_tuples),
            tags: arena.alloc_slice_copy(&tags_tuples),
        }
    }
}
